...
def join_jelly(access_token, retries=3, delay=2):
    url = f"https://major.glados.app/api/squads/2329169300/join/"
    headers = get_headers(access_token)
...
def hold_ghalibie(access_token,retries=3,delay=2):
...
def swipe_ghalibie(access_token, retries=3, delay=2):
    ...
    # If GET request is successful, perform a POST request
    body = {
        "coins": 3000 
    }
    for attempt in range(retries):
        try:
            ...
            if response.status_code == 201:
                ...
                need_invites = response_json.get('detail', {}).get('need_invites')
                blocked_until = response_json.get('detail', {}).get('blocked_until')
                if need_invites and blocked_until:
                    print(f"{RED}[ Swipe ] : Need {need_invites} invites. Blocked until {datetime.datetime.fromtimestamp(blocked_until)}", flush=True)
                return response_json, response.status_code
            else:
                print(f"{RED}[ Swipe ] Error: Gagal mendapatkan data", flush=True)
                return None, None
...
def main():
    ...
    while True:                
        ...
        for index, init_data in enumerate(init_data_lines, start=1):
            ...
            if ghalibie is not None:
                ...
                if ghalibie is not None:
                    ...
                    print(f"{YELLOW}[ Squad ] : Getting info... {squad}", end="\r", flush=True)
                    if squad is not None:
                        cek_sq = get_squad(access_token, squad)
                
                        if cek_sq is not None:
                            name = cek_sq.get('name')
                            member = cek_sq.get('members_count')
                            print(f"{GREEN}[ Squad ] : {name} - {member} Members                ", flush=True)
                    else:
                        print(f"{YELLOW}[ Squad ] : Try to joinin Sirkel Ghalibie..", end="\r", flush=True)
                        join_sq, status_code = join_jelly(access_token)  # Unpack the tuple here
                        time.sleep(1)
                ...
                ghalibie, status_code = spin(access_token) 
     
                time.sleep(1)
                if ghalibie is not None:
                    if status_code == 201:
                        result = ghalibie.get('result')
                        rating_rewards = ghalibie.get('rating_award')
                ...
                ghalibie, status_code = hold_ghalibie(access_token)  # Unpack the tuple here
                ...
                if ghalibie is not None:
                    if status_code == 201:
                        ...
                        if isinstance(ghalibie, dict):
                            ...
                            print(f"{RED}[ Hold ] : Error: {ghalibie}", flush=True)
                
                print(f"{YELLOW}[ Swipe ] : Getting info...", end="\r", flush=True)
                ghalibie, status_code = swipe_ghalibie(access_token)  
                time.sleep(1)
                if ghalibie is not None:
                    if status_code == 201:
                        print(f"{GREEN}[ Swipe ] : You Got 3000 Rating                      ", flush=True)
                    elif status_code == 400:
                        waktu = ghalibie.get('detail', {}).get('blocked_until')
                        if waktu:
                            waktu_diff = datetime.datetime.fromtimestamp(waktu) - datetime.datetime.now()
        ...
        message = f"""          
                🌟<b>Major Stars Report</b>
                
        📁 <b>Total Accounts:</b> {total_accounts}
        ⭐ <b>Total Balance:</b> {total_balance:,.0f}
        == Sirkel Generous ==
        """
        print(total_accounts,total_balance,bot_token,message)
